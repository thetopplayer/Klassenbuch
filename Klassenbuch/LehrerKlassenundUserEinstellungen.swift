    ...
    class LehrerKlassenundUserEinstellungen: UITableViewController {
        ...
        func getInfos(){
            ...
            handle = ref?.child("users").child("Lehrer").child(uid!).child("Klasse").observe(.value, with: { (snapshot) in
            ...
            ref = FIRDatabase.database().reference()
            handle = ref?.child("users").child("Lehrer").child(uid!).child("email").observe(.value, with: { (snapshot) in
